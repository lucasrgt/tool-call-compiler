...
export { INTENT_SCHEMA, PLAN_SCHEMA, RECIPE_SCHEMA } from "./schemas.gen.js";
import {
...
export type Json =
    null | boolean | number | string | RefValue | Json[] | { [key: string]: Json };
...
export type TraceStatus =
    "started" | "finished" | "cache_hit" | "failed" | "retried" | "skipped";
...
export type SkipReason = "condition" | "failed_dependency" | "cancelled" | "not_run";
...
export function compileRecipeWithParams(
    ...
    for (const [name, value] of Object.entries(params)) {
        if (value === null) {
            throw new Error(
                `recipe parameter '${name}' is required but was not supplied`,
            );
        }
    }
...
function compileFanOut(recipeValue: FanOutRecipe | MapReduceRecipe): NodeSpec[] {
    ...
    if (items.length === 0) {
        throw new Error(
            "fan_out recipe needs a non-empty 'items' array or an 'items_ref'",
        );
    }
...
function parseRef(reference: string): string {
    ...
    for (const segment of parts.slice(2)) {
        if (segment === "") {
            throw new Error(
                `value reference '${reference}' contains an empty path segment`,
            );
        }
    }
    return node;
...
function collectInto(value: Json, nodes: Set<string>): void {
    ...
    if ("$ref" in record) {
        ...
        if (typeof reference !== "string") {
            throw new Error(
                `'$ref' value must be a string, found ${JSON.stringify(reference)}`,
            );
        }
